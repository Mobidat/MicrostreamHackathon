...
public class MedicationPopup extends Dialog
{
	 ...
	 * @return the medication popup
	 */
	public MedicationPopup setCMBMedical(final MedicationEntity medication)
	{
		this.cmbMedical.setValue((MedicalEntity)PersonDAO.findPerson(medication.getMedicalId()));
		return this;
	}
	...
	{
		if(TreatmentDAO.isSaved(this.medicationEntity))
		{
			TreatmentDAO.removeEntity(this.medicationEntity);
		}
	...
	private void initUI()
	{
		...
		this.cmbMedication.setItemLabelGenerator(ItemLabelGeneratorFactory.NonNull(Medication::getCaption));
		...
		this.binder.forField(this.dateDate).asRequired().bind(MedicationEntity::getDate, MedicationEntity::setDate);
